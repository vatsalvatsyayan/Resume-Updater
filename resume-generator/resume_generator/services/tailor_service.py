"""
Tailoring service: uses an LLM to select and rewrite profile content
for a specific job description, producing a TailoredResume.
"""
...
def _profile_to_context(data: ResumeGeneratorInput) -> str:
    """Serialize profile to a compact text/JSON block for the prompt."""
    return json.dumps(
        {
...
- extraSections: array of {{ title: string, items: string[] }} (optional, e.g. "Relevant coursework")
...
class TailorService:
    """
    Generates a tailored resume from profile data and job description
    ...
    def __init__(
        ...
        self._llm = get_provider(llm_config)

    def tailor(self, data: ResumeGeneratorInput) -> TailoredResume:
        """
        Run the LLM to select and rewrite content, then parse into TailoredResume.

        Raises:
            ValueError: If the LLM response is not valid JSON or does not match schema.
        """
        ...
        return _dict_to_tailored_resume(obj)


def _dict_to_tailored_resume(obj: dict) -> TailoredResume:
    """Map parsed JSON to TailoredResume, with safe defaults for missing keys."""
    def get(key: str, default=None):
        if default is None and key in ("education", "workExperience", "projects", "skills", "certifications", "volunteer", "leadership", "extraSections"):
            default = []
        return obj.get(key, default) or default

    education = [
        TailoredEducation(**e)
        for e in (get("education") or [])
        if isinstance(e, dict)
    ]
    work = [
        TailoredWorkExperience(**w)
        for w in (get("workExperience") or [])
    ...
    if not isinstance(skills, list):
        skills = []
    certs = [
        TailoredCertification(**c)
        for c in (get("certifications") or [])
        if isinstance(c, dict)
    ]
    volunteer = [
        TailoredVolunteer(**v)
        for v in (get("volunteer") or [])
        if isinstance(v, dict)
    ]
    leadership = [
        TailoredLeadership(**l)
        for l in (get("leadership") or [])
        if isinstance(l, dict)
    ]
    extra = [
        TailoredSection(**s)
        for s in (get("extraSections") or [])
