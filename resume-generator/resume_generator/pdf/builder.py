...
def _pdf_safe(text: str) -> str:
    """Replace Unicode characters unsupported by Helvetica (en/em dash, smart quotes)."""
    if not text:
        return text
    return (
        text.replace("\u2013", "-")
...
class PDFBuilder:
    """Generates a one-page resume PDF from TailoredResume. Never exceeds one page."""
    ...
    def build(
        ...
        """
        Build PDF from tailored resume.

        Args:
            resume: TailoredResume instance.
            output: If provided, write PDF to this path and return the Path.
                    If None, return PDF as bytes.

        Returns:
            Either the path to the written file or the PDF bytes.
        """
        try:
        ...
        if resume.education and self._has_room(pdf, _MIN_SECTION_BUFFER_MM):
            ...
            for e in resume.education:
                if not self._has_room(pdf, 18):
                    break
                self._block_title(pdf, _pdf_safe(f"{e.courseName}, {e.major}"), _pdf_safe(e.universityName) if e.universityName else None)
        ...
        if resume.workExperience and self._has_room(pdf, _MIN_SECTION_BUFFER_MM):
            ...
            for w in resume.workExperience:
                if not self._has_room(pdf, 20):
                    break
                self._block_title(pdf, _pdf_safe(w.position), _pdf_safe(w.companyName) if w.companyName else None)
                date_str = self._date_range(w.startDate, w.endDate, w.isPresent)
                if date_str:
                    pdf.set_font("Helvetica", "", self.font_size_body - 1)
                    pdf.set_text_color(100, 100, 100)
                    pdf.cell(0, 4, _pdf_safe(date_str + (f" | {w.location}" if w.location else "")), new_x="LMARGIN", new_y="NEXT")
                    pdf.set_text_color(0, 0, 0)
                pdf.set_font("Helvetica", "", self.font_size_body)
                for b in w.bullets:
        ...
        if resume.projects and self._has_room(pdf, _MIN_SECTION_BUFFER_MM):
            ...
            for p in resume.projects:
                if not self._has_room(pdf, 20):
                    break
                self._block_title(pdf, _pdf_safe(p.projectName), _pdf_safe(p.link or ", ".join(p.techStack)) if (p.link or p.techStack) else None)
        ...
        if resume.volunteer and self._has_room(pdf, _MIN_SECTION_BUFFER_MM):
            ...
            for v in resume.volunteer:
                if not self._has_room(pdf, 15):
                    break
                self._block_title(pdf, _pdf_safe(v.role), _pdf_safe(v.organizationName) if v.organizationName else None)
                for b in v.bullets:
        ...
        if resume.leadership and self._has_room(pdf, _MIN_SECTION_BUFFER_MM):
            ...
            for l in resume.leadership:
                if not self._has_room(pdf, 15):
                    break
                self._block_title(pdf, _pdf_safe(l.title), _pdf_safe(l.organization) if l.organization else None)
                for b in l.bullets:
                    if not self._has_room(pdf, 6):
                        break
                    self._bullet(pdf, _pdf_safe(b))
    ...
    def _has_room(self, pdf, needed_mm: float) -> bool:
        """True if at least needed_mm space remains on the page (single-page limit)."""
